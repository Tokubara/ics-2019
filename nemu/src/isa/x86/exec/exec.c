...
/* 0xff */
make_group(gp5,
    EX(inc), EX(dec), IDEX(call_E, call), EMPTY,
    IDEX(call_E, jmp), EMPTY, EX(push), EMPTY)

/* 0x0f 0x00*/
make_group(gp6,
    EMPTY, EMPTY, EMPTY, EX(ltr),
    EMPTY, EMPTY, EMPTY, EMPTY);
...
static OpcodeEntry opcode_table[512] = {
  ...
  /* 0x64 */	EMPTY, EMPTY, EX(operand_size), EMPTY, // 对应的是0x66
  ...
  /* 0xf8 */	EMPTY, EMPTY, IDEXW(E, gp3, 1), IDEX(E, gp3),
  /* 0xfc */	EX(cld), EMPTY, IDEXW(E, gp4, 1), IDEX(E, gp5),

/* {{{2 2 byte_opcode_table */
  ...
  /* 0x1c */	EMPTY, EMPTY, EMPTY, EMPTY,
  /* 0x20 */	IDEX(lea_G2E, mov_cr2E), EMPTY, IDEX(mov_E2G, mov_E2cr), EMPTY,
...
void isa_exec(vaddr_t *pc) {
  ...
  decinfo.opcode = opcode;
  // Log_debug("pc:%x, opcode:%x", cpu.pc, opcode);
  set_width(opcode_table[opcode].width);
  idex(pc, &opcode_table[opcode]);
}
